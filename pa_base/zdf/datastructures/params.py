...
class BaseParams:
    ...
    **Fields**
        ...
        | ``sntw_categories`` -- sntw_categories allowed in result set ("Doku", ...) for ARD-aware genre filtering
        | ``exclude_sntw_categories`` -- sntw_categories NOT allowed in result set ("Doku", ...) for ARD-aware genre filtering
        ...
        | ``teaser_type`` -- allow only items with a specific type of teaser, e.g., group-cluster-poster for stage
              automation
        | ``content_owners`` -- allow only items with a specific content owner, e.g., "ARD", "ZDF", "ZDFinfo", ...
        | ``exclude_content_owners`` -- exclude items with a specific content owner, e.g., "ARD", "ZDF", "ZDFinfo", ..., excludes "ARD" by default
        | ``only_documentaries`` -- allow only items with sntw_category "Doku" - deprecated, use sntw_categories instead for ARD-aware genre filtering
        | ``video_fsks`` -- allow only items with a specific FSK rating, e.g., "none", "fsk6", "fsk12", "fsk16", "fsk18", or "" for unset FSK rating
        | ``exclude_video_fsks`` -- exclude items with a specific FSK rating, e.g., "none", "fsk6", "fsk12", "fsk16", "fsk18", or "" for unset FSK rating
        | ``exclude_sensitive_content`` -- TRUE BY DEFAULT: exclude items with sensitive content marked by editors with "loadRecommendations==False" in Sophora
    """
    ...
    exclude_sntw_categories: Collection[str] = attr.ib(default=(), converter=copy.deepcopy)
    ...
    brand_exclude_tags: Collection[str] = attr.ib(factory=tuple, converter=copy.deepcopy)
    brand_exclude_tags_exact_matching: bool = False
    types: Collection[str] = attr.ib(default=("clip", "episode"), converter=copy.deepcopy)
    ...
    accessibility_options: Set[str] = attr.ib(
        ...
        validator=attr.validators.deep_iterable(member_validator=attr.validators.in_(options={"ut", "ad", "dgs"})),
    )
    tivi_age_group: str = attr.ib(
        default="",
        converter=attr.converters.default_if_none(""),
        validator=attr.validators.in_(options={"", "2-5", "6-13"}),
    )
    exclude_tivi_age_groups: Collection[str] = attr.ib(
        default=(),
        converter=copy.deepcopy,
        validator=attr.validators.deep_iterable(member_validator=attr.validators.in_(options={"", "2-5", "6-13"})),
    )
    ...
    video_fsks: Collection[str] = attr.ib(
        default=(),
        converter=copy.deepcopy,
        validator=attr.validators.deep_iterable(
            member_validator=attr.validators.in_(options={"", "none", "fsk6", "fsk12", "fsk16", "fsk18"})
        ),
    )
    exclude_video_fsks: Collection[str] = attr.ib(
        default=(),
        converter=copy.deepcopy,
        validator=attr.validators.deep_iterable(
            member_validator=attr.validators.in_(options={"", "none", "fsk6", "fsk12", "fsk16", "fsk18"})
    ...
    padding: str = attr.ib(default="none", init=False, converter=attr.converters.default_if_none("none"))
    ...
    def update_with_configuration_dict(self, conf_dict: Mapping[str, Any], copy_conf_dict: bool = True):
        if copy_conf_dict:
        ...
        if "exclude_tags_exact_matching" in conf_dict:
            self.exclude_tags_exact_matching = conf_dict.get("exclude_tags_exact_matching")
        ...
        if "brand_exclude_tags_exact_matching" in conf_dict:
            self.brand_exclude_tags_exact_matching = conf_dict.get("brand_exclude_tags_exact_matching")
        ...
        if "exclude_content_owners" in conf_dict:
            # PERAUT-1244 ARD should be excluded by default if not explicitly allowed
            self.exclude_content_owners = conf_dict.get("exclude_content_owners", ("ARD",))
        if "only_documentaries" in conf_dict:
            self.only_documentaries = conf_dict.get("only_documentaries")
        if "video_fsks" in conf_dict:
            self.video_fsks = conf_dict.get("video_fsks") or ()
        if "exclude_video_fsks" in conf_dict:
            self.exclude_video_fsks = conf_dict.get("exclude_video_fsks") or ()
        if "exclude_sensitive_content" in conf_dict:
            self.exclude_sensitive_content = conf_dict.get("exclude_sensitive_content")
    ...
    **Fields** from :class:`BaseParams` plus the following:
    ...
    def update_with_configuration_dict(self, conf_dict: Mapping[str, Any], copy_conf_dict: bool = True):
        if copy_conf_dict:
            # prevent accidental modifications of configurations
            conf_dict = copy.deepcopy(conf_dict)
        super().update_with_configuration_dict(conf_dict=conf_dict, copy_conf_dict=False)
        if "target" in conf_dict:
            self.target = conf_dict.get("target")
        if "promotion_assets" in conf_dict:
            self.promotion_assets = conf_dict.get("promotion_assets") or []
...
class RecommendationBaseParams(AutomationParams):
    ...
    **Fields** from :class:`AutomationParams` plus the following:
    ...
    padding: str = attr.ib(default="none", converter=attr.converters.default_if_none("none"))
...
class ClusterlistParams(RecommendationBaseParams):
    ...
    **Fields** from :class:`RecommendationBaseParams` plus the following:
    ...
    def update_with_configuration_dict(self, conf_dict: Mapping[str, Any], copy_conf_dict: bool = True):
        ...
        if "cluster_list" in conf_dict and isinstance(conf_dict.get("cluster_list"), list):
