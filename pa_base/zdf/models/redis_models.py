...
from pa_base.models.util import combine_decayed
from pa_base.zdf.train.user_segmentation import GenreSegmentsEnum, default_segment

if not WITHOUT_REDIS_MODELS and (not REDISMODEL_HOST or not REDISMODEL_PORT):
    raise ValueError("REDISMODEL_HOST and REDISMODEL_PORT need to be defined")

# CONFIG
# TTL (timedelta or seconds -> convert to seconds so it can be used as default
# arg and spares conversion on each redis EXPIRE call)
...
# TTL for default event model
EVENT_RECOMMENDATION_TTL: Optional[int] = None

# TTL for event alternative/secondary A/B testing models
EVENT_AB_RECOMMENDATION_TTL: Optional[int] = int(timedelta(days=3).total_seconds())

# TTL for Channel Zapping scores
CHANNEL_ZAPPING_TTL: Optional[int] = int(timedelta(hours=26).total_seconds())
...
# CONNECTIONS
if not WITHOUT_REDIS_MODELS and REDISMODEL_HOST and REDISMODEL_PORT:
    _pool_args = dict(
        host=REDISMODEL_HOST,
        port=REDISMODEL_PORT,
    ...
    _POOL_DB4 = redis.ConnectionPool(db=4, **_pool_args)
    del _pool_args
    ...
    # redis-based pre-computed recommendations scores for events (DB 4)
    # use ModelSuffix (stage_cmab, stage_sbm) to distinguish the keys
    EVENTS_SCORES_REDIS: redis.Redis = redis.Redis(connection_pool=_POOL_DB4)
    del _POOL_DB4
else:
    logging.warning("REDISMODEL_HOST and REDISMODEL_PORT need to be defined, setting all redis models to 'None'.")
    SCORES_REDIS = None
    AUTOMATIONS_REDIS = None
    USERS_SEGMENTS_REDIS = None
    SEGMENTS_SCORES_REDIS = None
    EVENTS_SCORES_REDIS = None
...
class AutomationKey(str, Enum):
    ...
    # ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### #
    #              autmated Channel Zapping clusters              #
    # ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### #

    channel_00 = "channel_00"
    """channel zapping cluster for channel 0"""

    channel_01 = "channel_01"
    """channel zapping cluster for channel 1"""

    channel_02 = "channel_02"
    """channel zapping cluster for channel 2"""

    channel_03 = "channel_03"
    """channel zapping cluster for channel 3"""

    channel_04 = "channel_04"
    """channel zapping cluster for channel 4"""

    channel_05 = "channel_05"
    """channel zapping cluster for channel 5"""

    channel_06 = "channel_06"
    """channel zapping cluster for channel 6"""

    channel_07 = "channel_07"
    """channel zapping cluster for channel 7"""

    channel_08 = "channel_08"
    """channel zapping cluster for channel 8"""

    channel_09 = "channel_09"
    """channel zapping cluster for channel 9"""

    channel_10 = "channel_10"
    """channel zapping cluster for channel 10"""

    channel_11 = "channel_11"
    """channel zapping cluster for channel 11"""

    channel_12 = "channel_12"
    """channel zapping cluster for channel 12"""

    channel_13 = "channel_13"
    """channel zapping cluster for channel 13"""

    channel_14 = "channel_14"
    """channel zapping cluster for channel 14"""

    channel_15 = "channel_15"
    """channel zapping cluster for channel 15"""

    channel_16 = "channel_16"
    """channel zapping cluster for channel 16"""

    channel_17 = "channel_17"
    """channel zapping cluster for channel 17"""

    channel_18 = "channel_18"
    """channel zapping cluster for channel 18"""

    channel_19 = "channel_19"
    """channel zapping cluster for channel 19"""
...
@unique
class EventsModelSuffix(str, Enum):
    # ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### #
    #     model types used for event recommendations              #
    # ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### #

    event_cmab = ":event_cmab"
    """cmab for event recommendations"""

    event_cmab2 = ":event_cmab2"
    """cmab for event recommendations"""

    event_cmab3 = ":event_cmab3"
    """cmab for event recommendations"""
...
class _RedisBaseModel(AbstractContextManager, KnowsItemMixin):
    ...
    def __init__(self, *, r: redis.Redis = ...):
        # input validation
        if r is ...:
            raise ValueError("A redis instance needs to be provided to RedisModel!")
        elif r is None:
            raise ValueError("Providing 'None' as redis instance is not supported!")
        # instance attributes
        self.r = r
...
class RedisModel(_RedisBaseModel, PredictMixin, SimilarItemsMixin):
    ...
    def __init__(self, *, suffix: ModelSuffix, r: redis.Redis = ...):
        # input validation
        # valid_suffixes = {":cbm", ":tbm", ":sbm"}
        # if suffix not in valid_suffixes:
        #    raise ValueError(f"'suffix' needs to be one of '{valid_suffixes}'")
        if not isinstance(suffix, ModelSuffix):
            raise ValueError("'suffix' needs to be a ModelSuffix")
        if r is ...:
            r = SCORES_REDIS
        # instantiate parent
    ...
    def remove_items(self, keep_items=()) -> None:
        ...
        if keep_items is None or not any(keep_items):
            self._purge_redis(suffix=self.suffix.value)
        else:
            if self.pipeline is None:
                raise RuntimeError("No pipeline has been started!")
            redis_keys: List[str] = self._list_redis_keys(suffix=self.suffix)
            del_keys = set(redis_keys) - {itemid + self.suffix.value for itemid in keep_items}
            if any(del_keys):
                # UNLINK is similar to DEL, but works async in redis
    ...
    def add_knn_for_item(self, itemid: str, scores: Mapping[str, float]):
        ...
        self.pipeline.rpush(itemid + self.suffix.value, *[f"{k}:{v:.2f}" for k, v in scores.items()])
    ...
    def add_knn_for_all_items(self, item_score_mapping: Mapping[str, Mapping[str, float]]):
        """
...
class RedisAutomationModel(_RedisBaseModel, PredictMixin):
    ...
    def __init__(self, *, key: AutomationKey, r: redis.Redis = ...):
        # input validation
        # valid_keys = {"lcm", "pvm", "lvm", "t5m", "tdm"}
        # if key not in valid_keys:
        #     raise ValueError(f"'key' needs to be one of '{valid_keys}'")
        if not isinstance(key, AutomationKey):
            raise ValueError("'key' needs to be an AutomationKey")
        if r is ...:
            r = AUTOMATIONS_REDIS
        # instantiate parent
        _RedisBaseModel.__init__(self, r=r)
    ...
    def add_scores(self, scores: Mapping[str, float]):
        ...
        if not scores:
            return
        # store only 2 decimals of each score - redis only knows byte strings, so the datatype doesn't matter
        self.pipeline.rpush(self.key.value, *[f"{k}:{v:.2f}" for k, v in scores.items()])
        ttl: Union[None, int, timedelta] = (
            CHANNEL_ZAPPING_TTL if self.key.name.startswith("channel_") else AUTOMATION_TTL
        )
        if ttl is None:
            # persist the key, i.e., remove previously set ttl
...
class RedisUserSegmentModel(_RedisBaseModel):
    ...
    def add_segments_for_all_userid_hashes(self, user_segment_mapping: Mapping[str, Union[str, GenreSegmentsEnum]]):
        """
        add userid to segment mapping to redis (currently saves the segment's name, e.g., 'my_uuid'->'serien')
        ...
        user_segment_mapping: Mapping[str, str] = {
            userid_hash: (
                ...
                else (segment if segment in GenreSegmentsEnum.names else default_segment.name)
            )
...
class RedisStageRecommendationsModel(_RedisBaseModel):
    ...
    def __init__(self, *, suffix: StageModelSuffix, r: redis.Redis = ...):
        # input validation
        if not isinstance(suffix, StageModelSuffix):
            raise TypeError("'suffix' needs to be a StageModelSuffix")
        if r is ...:
    ...
    def remove_items(self, keep_items=()) -> None:
        ...
        if keep_items is None or not any(keep_items):
            ...
            if self.pipeline is None:
                raise RuntimeError("No pipeline has been started!")
            redis_keys: List[str] = self._list_redis_keys(suffix=self.suffix.value)
            del_keys = set(redis_keys) - {itemid + self.suffix.value for itemid in keep_items}
            if any(del_keys):
                # UNLINK is similar to DEL, but works async in redis
                self.pipeline.unlink(*del_keys)
...
class RedisEventRecommendationsModel(_RedisBaseModel):
    """
    #TODO CAREFUL: this part is duplicated / required in the cmab code
    A context manager for redis connections & keys for pre-computed model scores for events in redis.
    ...
    def __init__(self, *, suffix: StageModelSuffix, r: redis.Redis = ...):
        # input validation
        if type(suffix) is not EventsModelSuffix:
            raise TypeError("'suffix' needs to be a EventsModelSuffix")
        if r is ...:
    ...
    def remove_items(self) -> None:
        """Remove items matching ``self.suffix``."""
        self._purge_redis(suffix=self.suffix.value)

    def add_scores(self, scores: Mapping[str, float]):
        """Add model prediction to redis

        :param scores: A mapping of item IDs and its score `{<item_id_0>:<score>, <item_id_1>:<score>,...}`.
        """
        if self.pipeline is None:
            raise RuntimeError("No pipeline has been started!")
        # clear scores set before writing new ones
        self.pipeline.delete(self.suffix.value)

        # store only 2 decimals of each score - redis only knows byte strings, so the datatype doesn't matter
        self.pipeline.rpush(self.suffix.value, *[f"{k}:{v:.2f}" for k, v in scores.items()])
        ttl: Union[int, timedelta] = (
            # use default stage model TTL for default model stage_cmab
            EVENT_RECOMMENDATION_TTL
            if self.suffix == EventsModelSuffix.event_cmab
            # use alternative stage model TTL for other stage models
            else EVENT_AB_RECOMMENDATION_TTL
        )
        if ttl is None:
            # persist the key, i.e., remove previously set ttl
            self.pipeline.persist(self.suffix.value)
        else:
            self.pipeline.expire(self.suffix.value, ttl)
    ...
    def get_scores(
        ...
        *,
        ...
        """NOT EXECUTED IN PIPELINE - gets ``k`` scores (predictions)."""
        scores = [
            (lambda x: (x[0], float(x[1])))(v.rsplit(":", maxsplit=1))
            for v in self.r.lrange(
                self.suffix.value,
