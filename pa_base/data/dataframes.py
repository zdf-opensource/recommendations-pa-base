...
from pa_base.util import get_model_target

try:
    from pa_base.configuration.dynamic_configs import (
...
def get_denoised_data(
    *,
    ...
    data_ids: Optional[pd.DataFrame] = None
    ...
    data_cl: Optional[pd.DataFrame] = None
    if use_cl_data and cl_n_days:
        ...
        for frontend in cl_frontend:
            ...
            single_frontend_cl.drop_duplicates(
                subset=["externalid", "uid"], keep="first", inplace=True
            )
            logging.info(f"Duplicates removed. Row count {len(single_frontend_cl)}")
            # denoise #1: skip items with low coverage
            single_frontend_cl = single_frontend_cl[
                single_frontend_cl.coverage > coverage_threshold
            ]
            logging.info(
                f"Low coverage < {coverage_threshold} removed. Row count {len(single_frontend_cl)}"
            )
            # denoising #2: remove old entries
            ...
            single_frontend_cl = single_frontend_cl[
                single_frontend_cl.datetime_local > play_min_cl_lt
            ]
            logging.info(
                f"Old entries > {cl_n_days} removed. Row count {len(single_frontend_cl)}"
            )
            # denoising #3: remove bogus new entries (timestamp in future)
            single_frontend_cl = single_frontend_cl[
                single_frontend_cl.datetime_local < now_local
            ]
            logging.info(
                f"Bogus entries (in the future) removed. Row count {len(single_frontend_cl)}"
            )
    ...
    # ]
    data = data[
        data.externalid.map(data.externalid.value_counts(sort=False) >= count_threshold)
    ]
    logging.info(
    ...
    if user_count_threshold:
        ...
        # ]
        data = data[
            data.uid.map(data.uid.value_counts(sort=False) >= user_count_threshold)
        ]
    logging.info(
...
def get_raw_content_df(
    *,
    s3_bucket: str = CONTENT_SYNC_BUCKET_NAME,
    s3_input_key_prefix: str = CONTENT_SYNC_PREFIX,
    output_dir: str = None,
) -> pd.DataFrame:
    """
    downloads the latest CSV/JSON matching S3_input_key_prefix from S3_input bucket to output_dir

    **THIS IS THE RAW CONTENT EXPORT BEFORE ETL**

    >>> type(get_raw_content_df())
    'DataFrame'
    >>> from pa_base.configuration.config import CONTENT_SYNC_BRANDS_PREFIX
    ... get_raw_content_df().shape[1] == get_raw_content_df(
    ...     s3_input_key_prefix=CONTENT_SYNC_BRANDS_PREFIX,
    ... ).shape[1]
    True

    :param s3_bucket: source bucket
    :param s3_input_key_prefix: prefix of CSV, e.g., CONTENT_SYNC_PREFIX or CONTENT_SYNC_BRANDS_PREFIX
    :param output_dir: local download dir
    :return: dataframe with one content item per row
    """
    usecols = [
        "externalid",
        "id",
        ...
        "has_ad",
        "has_dgs",
        # "is_sports",  # no longer provided by data platform
        # "is_kids",  # no longer provided by data platform
        # "is_news",  # no longer provided by data platform
        "search_service_tagging_results",
        "programme_items",
        "prodnr",  # later renamed to prodNr as in FSDB df
        "tivi_age_group",
        "default_end_date",
        ...
        "vmp",
        "is_partnercontent",
        "content_owner",  # only given for partner content, empty for ZDF
        "is_doku",  # SNTW-87 flag is true for documentations of both ARD and ZDF (c.f., content_owner)
        "sntw_category",
    ]

    # read downloaded file
    def transform_df(df: pd.DataFrame, filetype: str) -> pd.DataFrame:
        """
        apply some filetype-specific transformations on the dataframe

        :param df: the downloaded dataframe
        :param filetype: filetype of the downloaded file, e.g., json.gz or csv
        :return: the transformed dataframe
        """
        if filetype == "json.gz":
            # json has some nested properties in programme_items
            df["airtimebegin"], df["airtimeend"] = zip(
                *df["programme_items"].apply(
                    lambda programme_items: (
                        programme_items[0].get("broadcasts")[0].get("airtimebegin"),
                        programme_items[0].get("broadcasts")[0].get("airtimeend"),
                    )
                    if isinstance(programme_items, list)
                    and len(programme_items) > 0
                    and isinstance(programme_items[0].get("broadcasts"), list)
                    and len(programme_items[0].get("broadcasts")) > 0
                    else (None, None)
                )
            )
            df["episode_number"] = df["programme_items"].apply(
                lambda programme_items: (programme_items[0] or {}).get(
                    "episode_number", 0.0
                )
                if isinstance(programme_items, list) and len(programme_items) > 0
                else 0.0
            )
            df["season_number"] = df["programme_items"].apply(
                lambda programme_items: (
                    (programme_items[0] or {}).get("Season") or {}
                ).get("season_number", 0.0)
                if isinstance(programme_items, list) and len(programme_items) > 0
                else 0.0
            )
            df["country"] = df["programme_items"].apply(
                lambda programme_items: (programme_items[0] or {}).get("country", "")
                if isinstance(programme_items, list) and len(programme_items) > 0
                else ""
            )
            df["actor_details"] = df["programme_items"].apply(
                lambda programme_items: (programme_items[0] or {}).get(
                    "actor_details", []
                )
                if isinstance(programme_items, list) and len(programme_items) > 0
            ...
            df["crew_details"] = df["programme_items"].apply(
                lambda programme_items: (programme_items[0] or {}).get(
                    "crew_details", []
                )
                if isinstance(programme_items, list) and len(programme_items) > 0
            ...
            df["text"] = df["programme_items"].apply(
                lambda programme_items: (programme_items[0] or {}).get("text", "")
                if isinstance(programme_items, list) and len(programme_items) > 0
                else ""
            )
        elif filetype == "csv":
            ...
            raise ValueError(f"Cannot load content from filetype '{filetype}'.")
        return df

    # download latest cf dump
    df = download_dataframe_from_s3(
        s3_bucket=s3_bucket,
        s3_key_prefix=s3_input_key_prefix,
        usecols=usecols,
        datecols=[
            "enddate",
            "publication_date",
    ...
    df.rename(columns={"prodnr": "prodNr"}, copy=False, inplace=True)
    return df


def get_content_df(s3_bucket: str = "", s3_key: str = ""):
    """get latest content as pandas dataframe"""
    if not s3_key:
        with s3_file(
            s3_bucket=s3_bucket or S3_PREPROCESSED_METADATA,
            s3_key_prefix="",
            s3_key_suffix="_content",
            filetypes=("parquet",),
        ) as f:
            content_df = pd.read_parquet(f)
    else:
        with s3_file(
            s3_bucket=s3_bucket or S3_PREPROCESSED_METADATA,
            s3_key=s3_key,
            ...
            content_df = pd.read_parquet(f)
    json_columns = ["zdfinfo_metadata", "search_service_tagging_results"]
    for column in json_columns:
        content_df[column] = content_df[column].apply(json.loads)
    return content_df
    ...
    for s3_key_index, prefix in enumerate(s3_prefixes):
        ...
        if not df.empty:
            ...
            for col in dataframe.select_dtypes(include="category").columns:
                all_cats = df[col].cat.categories | dataframe[col].cat.categories
...
def get_agg_cl_short_term_most_viewed_df() -> pd.DataFrame:
    ...
    for prefix_index, prefix in enumerate(s3_prefixes):
        try:
            logging.debug(
                f"Loading short-term most-viewed from S3 '{CLICKSTREAM_BUCKET_NAME}:{prefix}'."
            )
            ...
            dataframe.count_distinct_views = dataframe.count_distinct_views.astype(
                np.float32
            )
            ...
            logging.warning(
                "Error loading one shard of short-term most-viewed data.", exc_info=exc
            )
    logging.info(
        f"Loaded short-term most-viewed data of {len(daily_dfs)} days. Requested: {n_days} days."
    )
    if not daily_dfs:
    ...
    df["target"] = (
        "https://www.zdf.de" + df["path"].astype(str) + "/" + df["id"] + ".html"
    )
...
def get_interactions_df(tmpdir="/tmp"):
    ...
    def timestamp_to_int(df: pd.DataFrame) -> pd.DataFrame:
        df["timestamp"] = pd.to_numeric(
            df["timestamp"], errors="coerce", downcast="integer"
        )
        return df
...
def get_recos_data() -> pd.DataFrame:
    ...
    logging.info(
        f"Loading recommendations data from S3 '{CLICKSTREAM_BUCKET_NAME}:{s3_prefix}'."
    )
